...
**图片说明：** 此图展示了Basic Info编辑弹窗的界面设计。弹窗采用白色背景的模态设计，标题为"Basic Info"。主体内容包含三个主要配置项：1）Name输入框，显示默认值"🏆 Milestone Name"，右侧显示字符计数"30"；2）Start时间配置，包含时间选择器和"Start Instantly"勾选选项；3）End时间配置，包含时间选择器和"No End Time"勾选选项。弹窗底部有"Cancel"和"Confirm"两个操作按钮。整个界面设计简洁清晰，便于用户快速配置基本信息。
...
**图片说明：** 此图展示了Rewards模块的编辑按钮界面。显示一个标题为"Rewards"的区域，右侧有一个编辑图标按钮，用于触发奖励配置弹窗。界面设计简洁，编辑按钮采用常见的铅笔图标，便于用户识别和操作。

点击该编辑按钮后显示弹窗:

![Rewards配置弹窗](media/milestone/image5.png)

**图片说明：** 此图展示了Rewards配置弹窗的完整界面。弹窗标题为"Rewards"，顶部有"Milestone Metric"下拉选择器，默认显示"TaskOn EXP"选项。主体是一个数据表格，包含"Required XP"、"Reward"和操作列。表格显示了多个里程碑节点，如"100 TaskOn EXP"对应"10 USDT"奖励等。右侧有编辑和删除按钮。表格底部有"+ Add reward"按钮用于添加新的奖励节点。弹窗底部是"Cancel"和"Save"操作按钮。整个界面设计专业，便于配置多层级的里程碑奖励体系。
...
**图片说明：** 此图展示了Add rewards弹窗的奖励类型选择界面。弹窗标题为"Add rewards"，主体显示四种奖励类型选项：Token、NFT、Discord Role和PoAP，每种类型都有相应的图标和清晰的标识。用户可以点击任一选项来配置对应类型的奖励。界面设计简洁明了，便于用户快速选择所需的奖励类型。
...
**图片说明：** 此图展示了Participation Requirements参与要求设置区域。界面显示"Participation Requirements"标题，右侧有一个切换开关控件，用于启用或禁用参与条件限制。开关采用标准的toggle设计，便于用户快速开启或关闭参与要求功能。
...
**图片说明：** 此图展示了Participation Requirements的详细配置面板。面板从右侧滑出，包含两个主要配置区域：1）Anti-bot板块，复用community anti bot功能；2）Community Hub User Selection板块，包含多种用户筛选条件如Hold Token、Hold NFT、Quest Participation、Milestone等选项。每个选项都有相应的配置界面，用户可以设置具体的参与门槛和条件。界面设计层次清晰，便于进行精确的用户群体定位。
...
**图片说明：** 此图展示了B端里程碑列表页的完整界面。页面采用表格布局，包含多个列：里程碑名称、DURATION（持续时间）、METRIC（计算单位）、PARTICIPANTS（参与人数）、COMPLETED USERS（完成用户）和TOKEN BALANCE（代币余额）。表格中显示了具体的里程碑数据，如"Summer Gem Collection"项目的各项统计信息。每个里程碑卡片都有相应的操作按钮和状态指示。界面设计清晰，信息展示全面。
...
**图片说明：** 此图展示了Token充值弹窗的配置界面。弹窗标题为"Add Additional Reward"，主体包含多个输入配置项：顶部有描述文案"How many additional users do you want to reward?"，下方有数量输入框、代币类型选择器和金额输入框。底部显示总计金额和"Cancel"、"Add Reward"操作按钮。整个界面设计简洁实用，便于项目方快速补充奖励池。
...
![](media/milestone/image17.png){width="5.75in"
...
![](media/milestone/image20.png){width="5.75in"
height="2.71875in"}
...
**图片说明：** 此图展示了积分类型的里程碑进度条界面。进度条显示当前用户的积分进度和完成状态，设计简洁清晰，便于用户直观了解自己在积分类型里程碑中的完成情况。

等级类型的进度条:

![等级类型进度条](media/milestone/image23.png)

**图片说明：** 此图展示了等级类型的里程碑进度条界面。进度条显示当前用户的等级进度和完成状态，与积分类型相比有着不同的显示风格。
...
**图片说明：** 此图展示了里程碑Tab页的完整界面布局。页面包含用户当前的积分和等级信息、用户参与的里程碑进度，以及历史上每一个里程碑的详细信息。排序规则优先显示live状态，其次upcoming，最后end。
...
**图片说明：** 此图展示了当用户不符合参与条件时里程碑的显示样式。里程碑卡片采用灰化效果，并显示"check requirement"按钮，提醒用户需要满足特定条件才能参与。

**点击 check requirement 后显示 Eligibility 不符合条件弹窗:**

![Eligibility不符合条件弹窗](media/milestone/image30.png)

**图片说明：** 此图展示了Eligibility不符合条件的弹窗界面。弹窗标题为"Before you join this activity please meet ALL the following Eligibilities"，列出了用户需要满足的具体条件，并提供刷新按钮供用户重新检查。
...
**图片说明：** 此图展示了当用户符合参与条件时里程碑的显示效果。里程碑卡片正常显示，用户可以点击Complete按钮进行后续操作。

点击 Complete 后跳出弹窗:

![连接账户引导弹窗](media/milestone/image32.png)

**图片说明：** 此图展示了引导用户连接账户的弹窗界面。弹窗提示用户在参与里程碑之前需要满足相关要求，并列出了需要连接的各种账户类型。底部有Verify按钮用于校验是否符合条件。
